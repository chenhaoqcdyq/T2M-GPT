...
def setup(rank, world_size):
    ...
    os.environ['MASTER_PORT'] = str(random.randint(29500, 29550))
    # 添加NCCL相关的环境变量
...
def main(rank, world_size, args):
    ...
    # # 设置较小的批处理大小
    # if args.batch_size > 32:  # 如果批处理大小大于32，则减小
    #     args.batch_size = args.batch_size // 2
    #     if rank == 0:
    #         print(f"Reducing batch size to {args.batch_size} due to memory constraints")
    ...
    trans_encoder = trans_encoder.to(rank)
    ...
    trans_encoder = DDP(trans_encoder, device_ids=[rank], find_unused_parameters=True)
    trans_encoder.train()
    ...
    while nb_iter <= args.total_iter:
        ...
        if sem_tokens_len is not None:
            cls_pred = trans_encoder(a_indices, feat_clip_text, semantic_valid_lengths=sem_tokens_len)
        else:
        ...
        if nb_iter % args.eval_iter == 0:
            if rank == 0:
                torch.save({
                    'trans': trans_encoder.module.state_dict(),
